...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    console.log(`GET /api/festivals - Successfully fetched ${festivals.length} festivals`);
    res.json({
      success: true,
      data: { festivals },
...
router.get('/active', async (req, res) => {
  ...
  try {
    ...
    console.log('GET /api/festivals/active - Successfully fetched active festival:', festival.name);
...
router.get('/:id', async (req, res) => {
  console.log('GET /api/festivals/:id - Fetching festival with ID:', req.params.id);
  try {
    const festival = await festivalService.getFestivalById(req.params.id);
    console.log('GET /api/festivals/:id - Successfully fetched festival:', festival.name);
    res.json({
      success: true,
      data: { festival },
...
router.post('/', auth, async (req, res) => {
  console.log('POST /api/festivals - Creating new festival by user:', req.user.email);
  console.log('POST /api/festivals - Request body received:', JSON.stringify(req.body, null, 2));

  try {
    const { name, dates, description, location, ticketUrl, isActive } = req.body;
    ...
    console.log('POST /api/festivals - Festival data to save:', JSON.stringify(festivalData, null, 2));

    const festival = await festivalService.createFestival(festivalData);
    console.log('POST /api/festivals - Festival created successfully:', festival.name);
    res.status(201).json({
      success: true,
      data: { festival },
...
router.put('/:id', auth, async (req, res) => {
  console.log('PUT /api/festivals/:id - Updating festival with ID:', req.params.id);
  console.log('PUT /api/festivals/:id - Request body received:', JSON.stringify(req.body, null, 2));
  ...
  try {
    ...
    console.log('PUT /api/festivals/:id - Update data:', JSON.stringify(updateData, null, 2));

    const festival = await festivalService.updateFestival(req.params.id, updateData);
    console.log('PUT /api/festivals/:id - Festival updated successfully:', festival.name);
    res.json({
      success: true,
      data: { festival },
...
router.delete('/:id', auth, async (req, res) => {
  console.log('DELETE /api/festivals/:id - Deleting festival with ID:', req.params.id);
  try {
    const festival = await festivalService.deleteFestival(req.params.id);
    console.log('DELETE /api/festivals/:id - Festival deleted successfully:', festival.name);
    res.json({
      success: true,
      data: { festival },
